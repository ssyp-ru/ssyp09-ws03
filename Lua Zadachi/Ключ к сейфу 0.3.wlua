...
function randomnums()
	for i = 1, n do
		...
		r[i] = z[j]
		table.remove(z,j)
--		iup.Message('rootmsg','Нужное число - '..r[i])
	end
end
...
function guessing ()
	...
	ret,str = iup.GetParam('Код к сейфу: ввод',nil,'Введите '..n..' разных цифр с 0 по 9 (без пробелов или символов разделения). %s\n','')
	for i = 1, n do
		m[i] = 0+string.sub(str,i,i)
		for j = 1, i-1 do
			if m[i] == m[j] then
				iup.Message('Код к сейфу: ошибка','Ошибка: цифры не должны повторяться.')
				guessing()
			end
		end
--		iup.Message('rootmsg','Распознанное число - '..m[i])
	end
	y = 0
	v = 0
	for i = 1, n do
