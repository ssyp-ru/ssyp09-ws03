...
-----------------------------------------
function add()
	res, ob = iup.GetParam('Создать',nil,'Выберите объект: %l|Сорнячок|Ромашка|Одуванчик|Плоский червь|Дождевой червь|Дятел|Заяц|\n',0)
	if ob == 0 then
	...
	elseif ob == 3 then
		o_cherv:create(math.random(1,g_h),math.random(1,g_w),math.random(32,223),math.random(32,223),math.random(32,223))
	elseif ob == 4 then
		...
		iup.hbox
		{
			iup.vbox
			{
				iup.hbox
				{
					...
					iup.button{title='Создать...'; size='60x15', action=add},
