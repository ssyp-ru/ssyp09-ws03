...
function trava_zhivi(t,c)
	...
	if (t.razmnozh == 0) then
		...
		for p = 1,math.random(3) do
			...
			local nx = math.random(t.x-2,t.x+2)
			if nx<1 then nx = 1 end
			if nx>10 then nx = 10 end

			local ny = math.random(t.y-2,t.y+2)
	...
	--pr = "Возраст: "..t.vozrast-1 .." Осталось жить:"..t.max_vozrast - t.vozrast+1 .. " До потребления еды осталось "..t.sitost + 1 .." циклов".." Едениц еды осталось в почве:"..tgumus[t.x][t.y].." До следующего размножения осталось "..t.razmnozh + 1 .." циклов\n"
	--printin(pr)
