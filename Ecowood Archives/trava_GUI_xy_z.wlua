...
z_trava[trava[num].x][trava[num].y] = 1
end
...
function razmn(t)
	if (t.d_razmn < 1) and (t.sem > 0) then
		...
		for j = 1, t.sem do
			new_x, new_y = count_razm(t.x,t.y)
			create_grass(table.maxn(trava)+1,new_x,new_y,math.random(45,65),math.random(8,10),1,math.random(21,35),math.random(1,2),math.random(2,4))
			io.write('Воспроизвёл потомка в координатах: x-'..new_x..', y-'..new_y..'.\n')
		end
...
function count_razm(old_x, old_y)
local new_x = nil
local new_y = nil
for i = old_x-1, old_x+1 do
	for j = old_y-1, old_y+1 do
		if z_trava[i][j] ~= nil then
			if z_trava[i][j] == 0 then
				return i,j
			end
		end
	end
...
function new()
	...
	st_dead = 0
	gumus_r()
	create_grass(1,1,1,65,9,1,28,2,3)
	cycle()
end
function cycle()
	if day == nil then
