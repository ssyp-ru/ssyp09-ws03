...
function o_trava:eda()
	if self.sitost < 1 then
		...
		if gumus[self.x][self.y] > 0 then
			...
			grid:setcell(self.x,self.y,gumus[self.x][self.y]..' s'..string.sub(self.name,string.len(self.name),string.len(self.name)))
		else
			self:umri()
		end
	end
...
function o_sornyachok:create(x,y)
	...
	st_alive = st_alive + 1
	grid:setcell(x,y,grid:getcell(x,y)..' s'..string.sub(tmp_obj.name,string.len(tmp_obj.name),string.len(tmp_obj.name)))
end
------------------/Функции сорнячка----------------
