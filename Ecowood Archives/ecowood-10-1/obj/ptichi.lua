...
function o_ptichi:eda()
	if (self.sitost < self.sitost/2) and (self.statys ~= 'Мертв') and (self.statys ~= 'Разложился') then
...
function o_ptichi:poisk_edi()
	if (self.statys ~= 'Мертв') and (self.statys ~= 'Разложился') and (self.edagertva == nil) then
		for i = 1, table.maxn(life) do
			if (life[i].typ == 'червь') and (self:vizhu_object(life[i])) then
			local c = i
	...
	if (self.statys ~= 'Мертв') and (self.statys ~= 'Разложился') then
		...
		if (self.d_go <= 1) then -- ДА, можно
			...
			elseif (self.statys == 'Хочу есть') then
				...
				-- Откуда мы возьмем x1 ????
				-- если бы было так: x1,y1 = self:poisk_edi()
				if self.x < x1 then
					self.sx = 1
				elseif self.x > x1 then
					self.sx = -1
