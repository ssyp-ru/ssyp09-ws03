	...
	if (self.statys ~= 'Мертв') and (self.statys ~= 'Разложился') then
		...
		if (self.d_go <= 1) then -- ДА, можно
			...
			elseif (self.statys == 'Хочу есть') then
				...
				self.statys = 'Прогулка' -- на след ходу начнет гулять   ИЗМ
			end
...
function o_chervi:go()
	if (self.statys ~= 'Мертв') and (self.statys ~= 'Разложился') then
		...
		if (self.d_go <= 1) then -- ДА, можно
			...
			if (self.vozrast > self.max_vozrast - (self.max_vozrast/2)) and (self.razmnvoz == false) then
