...
o_chervi.othodi = 1
o_chervi.cikl_videl = 15 -- через сколько циклов выделяет ГУМУС в почву
...
o_chervi.razmnvoz = false -- чтобы размножались 1 раз.

function o_chervi:eda()
	if (self.sitost < 10) and (self.statys ~= 'Мертв') and (self.statys ~= 'Разложился') then
...
function o_chervi:razmn()
	if (self.vozrast >= (self.max_vozrast/2)) and (self.razmnvoz  == false) and (self.deti > 0) and (self.statys ~= 'Мертв') and (self.statys ~= 'Разложился') then
